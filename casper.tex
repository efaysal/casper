...
%To-add
%Minor:
%Target number of validators by picking the deposit size
%Pay a fee to top-up and to bond, so that there's no incentive to censor bonding
%Critical:
%Require a minimum number of validation signatures for a block to have its tx's executed OR only the one with the most validations is executed, to prevent a late-block-addition priority-attack. 
...
%I need to talk to Andrew Miller and Dominic Williams to get a better understanding of how to conduct this analysis
%Or I can read literature - that's a good idea!
...
%Outstanding issues:
%What if an attacker bonds a lot and is faulty with a bit, in order to control more of the bonded stake?
%Then they will have an easier time censoring txs, but they still cannot revert anything
