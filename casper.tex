...
If a client does not know the current set of bonded validators, then the fork-choice rule returns an exception. In this case, the client must learn and authenticate the validator set out-of-band. Otherwise, the client will be able to make a fork-choice. We will define a couple of terms before we specify Casper's fork-choice rule: 
